...
│     │   └── ⚠️ Dual Thrust 日线算法，盘中不做状态判断              │
│     └── K线收盘 (x=true): _on_kline_closed()                     │
│         └── 重新计算轨道，更新市场状态                            │
│                                                                  │
│  方式2: 1h K线聚合（备选）                                        │
│                                                                  │
...
Dual Thrust 是日线级别算法，但 Binance WebSocket 订阅 `@kline_1d` 会同时推送：
- 盘中更新（`x=false`）：实时价格变动，高频推送
- 收盘事件（`x=true`）：日线完成，每天一次

盘中判断存在以下问题：
1. 绕过算法的日线确认参数（`down_confirm_days`、`cooldown_days`）
2. 状态变化过于频繁（价格在轨道边界波动）
3. 与算法设计意图不符
...
| 方案 | 描述 | 优点 | 缺点 |
|------|------|------|------|
| **A（已选）** | 保持订阅，忽略盘中判断 | 简单、稳定、符合算法设计 | 收盘才更新状态 |
| B | 保持订阅，盘中也判断 | 提前感知趋势变化 | 参数语义混乱、状态不稳定 |
| C | 移除订阅，定时任务检查 | 减少消息量 | 收盘检测有延迟、时区问题 |

**选择方案 A 的原因**：

1. Dual Thrust 设计用途是"趋势过滤器"，用于判断当天是否适合开仓
2. 日线参数（`down_confirm_days`、`cooldown_days`）只在收盘计算时有效
3. 状态每天最多变化一次，符合算法稳定性要求
4. WebSocket 仍保留用于收盘事件实时触发

**未来扩展方向**：

如果需要盘中风险控制，可设计独立的盘中判断方法：
- 新增参数：`intraday_breakout_threshold`（盘中突破幅度阈值）
- 新增参数：`intraday_confirm_count`（盘中确认次数）
- 与日线逻辑分离，互不干扰
...
*文档版本: 1.3*
*创建日期: 2026-04-03*
*更新日期: 2026-04-06 - 添加盘中判断方案选择说明，修复历史数据周期动态获取*